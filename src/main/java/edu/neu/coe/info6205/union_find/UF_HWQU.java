 ...
 */
public class UF_HWQU implements UF {
    /**
     ...
     */
    public UF_HWQU(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
     ...
     * @throws IllegalArgumentException if {@code n < 0}
     */
    public UF_HWQU(int n) {
        this(n, false);   //without path compression
    }
    ...
    private void doPathCompression(int i) {
        ...
        if (root != parent[root]) {
            ...
            //update height when changing parent
            height[root] -= height[i];
        }
    }
}
